...
public class WaterLevelAndEffectRecipe implements Recipe<Inventory>, Comparable<WaterLevelAndEffectRecipe> {
    ...
    /*
    public List<MobEffectInstance> getMobEffectInstances() {
        return this.fluid != null && this.fluid.isSame((Fluid)FluidRegistry.SUSPICIOUS_LOOKING_WATER.get()) && Math.random() < (Double)ConfigRegistry.SUSPICIOUS_LOOKING_WATER_NEGATIVE_EFFECT_RATE.get() ? List.of(new MobEffectInstance((MobEffect) ForgeRegistries.MOB_EFFECTS.getValue(new ResourceLocation((String)ConfigRegistry.SUSPICIOUS_LOOKING_WATER_NEGATIVE_EFFECT.get())), (Integer)ConfigRegistry.SUSPICIOUS_LOOKING_WATER_NEGATIVE_EFFECT_DURATION.get())) : this.mobEffectInstances;
    }*/
    ...
    public boolean conform(ItemStack conformStack) {
        ...
        if (!ingredient.isEmpty()){
            boolean i = true;
            ItemStack[] var5 = this.getIngredient().getItems();
            int var6 = var5.length;

            for (int var7 = 0; var7 < var6; ++var7) {
                ItemStack ingredientStack = var5[var7];
